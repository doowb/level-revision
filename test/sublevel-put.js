...
var _db = null;
var db = null;
describe('LevelRevision with LevelRevisionSublevel', function () {
  beforeEach(function (done) {
    rimraf(dataPath, function (err) {
      if (err) return done(err);
      mkdirp.sync(dataPath);
      _db = level(dbPath);
      db = new LevelRevision(_db, { strategy: LevelRevisionSublevel });
      done();
    });
  });
  ...
  it('should put an item in the database', function (done) {
    db.put(['test-item-1'], { id: 'item-1', description: 'Item 1'}, function (err, item) {
      ...
      db._strategy._revisions.get(['test-item-1', 1], function (err, revision) {
        if (err) return done(err);
